        ...
        if (viewGroup != null && currentAct != null) {
            ...
            enterLocBtn.setOnClickListener(v -> {
                ...
                if (currentAct instanceof FoodDeliveryHomeActivity) {
                    new StartActProcess(currentAct).startActForResult(SearchLocationActivity.class, bn, Utils.UBER_X_SEARCH_PICKUP_LOC_REQ_CODE);
                } else {
            ...
            if (!generalFunc.isLocationEnabled()) {
                ...
                if (currentAct instanceof FoodDeliveryHomeActivity && (!((FoodDeliveryHomeActivity) currentAct).latitude.equalsIgnoreCase("0.0") || !((FoodDeliveryHomeActivity) currentAct).longitude.equalsIgnoreCase("0.0"))) {
                    closeView(generalFunc);
                    isViewExecutionLocked = false;
                    return;
                }
                if (currentAct instanceof ServiceHomeActivity && (!((ServiceHomeActivity) currentAct).latitude.equalsIgnoreCase("0.0") || !((ServiceHomeActivity) currentAct).longitude.equalsIgnoreCase("0.0"))) {
                    closeView(generalFunc);
                    isViewExecutionLocked = false;
                    return;
                }
